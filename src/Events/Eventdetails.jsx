...
const Eventdetails = () => {
  ...
  const handleTicketBuy = (e) => {
    ...
    if (!lsStoreEvent) {
      ...
      toast.success("You have successfully bought ticket for this event!");
    } 
    else{
      const isExist = lsStoreEvent?.find((lsId) => lsId === id);
      if (!isExist) {
        newArray.push(...lsStoreEvent,id)
        localStorage.setItem("booked", JSON.stringify(newArray))
        toast.success("You have successfully bought ticket for this event!");
      } else {
        toast.error("You have already booked ticket for this event!");
      }
    }
  };
  return (
      ...
      <div className="hero-overlay rounded-lg bg-opacity-60">
        <div className="md:flex md:px-10 px-4 items-center  justify-between">
          <div className="md:w-6/12 p-2 md:my-[10%]">
            ...
            <Link to={"/"}>
              {" "}
              <button className="btn mt-6  hidden lg:flex items-center">
          ...
          <form onSubmit={handleTicketBuy} className="flex flex-col md:w-4/12 space-y-5 mx-auto">
            ...
            >
              ...
              <img
                src="https://i.ibb.co/Z6GshGq/paypal-logo.png"
                className="w-[70px]"
                alt=""
              />
            </button>
