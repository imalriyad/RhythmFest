...
const EventCard = ({ card, btntitle }) => {
  ...
  return (
    <div className="shadow-md border rounded-md relative p-5">
      ...
      <h1 className="mt-5 text-xl font-semibold">{eventName}</h1>
      ...
      {btntitle ? (
        <Link state={id} to={`/eventdetails/${id}`}>
          {" "}
          <button className="btn left-[5%] my-5 absolute bottom-0 text-white hover:bg-[#0043a7] bg-maincolor w-[90%]">
            {" "}
            <IoTicketSharp className="text-2xl text-green-500" />
            Ticket Booked
          </button>
        </Link>
      ) : (
